...
const ComponentToImg = (props) => {
    console.log(props.children.props.theme)
    ...
    const downloadImage = async () => {
        ...
        let data
        if (props.children.props.theme == 'custom' || props.children.props.theme == 'background' || props.children.props.theme == 'stylish') {
            data = await domtoimage.toPng(componentRef.current, {
                height: element.offsetHeight * 2 * 0.99,
                width: element.offsetWidth * 2 * 0.99,
                style: {
                    transform: "scale(2) translate(-2px, -2px)",  // 偏移去白边
            ...
            data = await domtoimage.toPng(componentRef.current, {
                ...
                style: {
                    transform: "scale(2) translate(0px, 0px)",  // 偏移去白边
                    transformOrigin: "top left",
                    width: element.offsetWidth * 2 + "px",
                    height: element.offsetHeight * 2 + "px",
                }
        ...
        <React.Fragment>
            ...
            <button
				...
				<span>
					{
                        loading ?
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-white animate animate-spin"
